...
export async function analyzeResume(data: z.infer<typeof analyzeResumeSchema>) {
  try {
    ...
    if (!resume.content || typeof resume.content !== 'object' || !('text' in resume.content)) {
      return { success: false, error: 'Resume content not available for analysis' }
    }

    const resumeText = (resume.content as any).text as string

    if (!resumeText || resumeText.trim().length < 50) {
      return { success: false, error: 'Resume text is too short for meaningful analysis' }
    }    // Perform AI analysis
    ...
    const analysisRecord = await db.resumeAnalysis.create({
      data: {
        ...
        sections: analysis.sections,        keywords: {
          found: analysis.atsCompatibility?.issues || [],
          missing: analysis.missingElements || [],
          suggestions: analysis.improvements || []
        },
        suggestions: analysis.improvements?.map((improvement: string, index: number) => ({
          category: 'improvement',
          priority: index < 3 ? 'high' : 'medium' as 'high' | 'medium' | 'low',
  ...
  try {
    ...
    const analysisRecord = await db.resumeAnalysis.create({
      data: {
        ...
        suggestions: matchAnalysis.recommendations?.map((rec: string, index: number) => ({
          category: 'job_match',
          priority: index < 3 ? 'high' : 'medium' as 'high' | 'medium' | 'low',
...
export async function matchResumeToJobAdHoc(data: z.infer<typeof matchJobAdHocSchema>) {
  try {
    const session = await getDevSession()
...
export async function optimizeResume(data: z.infer<typeof optimizeResumeSchema>) {
  try {
    const session = await getDevSession()
    ...
    if (!resume.content || typeof resume.content !== 'object' || !('text' in resume.content)) {
      return { success: false, error: 'Resume content not available for optimization' }
    }

    const resumeText = (resume.content as any).text as string

    if (!resumeText || resumeText.trim().length < 50) {
      return { success: false, error: 'Resume text is too short for optimization' }
    ...
    const analysisRecord = await db.resumeAnalysis.create({
      data: {
        resumeId: resume.id,
        userId: session.user.id,
        type: 'OPTIMIZATION',        score: undefined, // Optimization doesn't have a numeric score
        sections: undefined,
        keywords: undefined,
        suggestions: Array.isArray(optimization) 
          ? optimization.map((opt: any, index: number) => ({
